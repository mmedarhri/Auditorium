...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
	xmlns:wsdl="http://schemas.xmlsoap.org/wsdl/" xmlns:isc="http://smartclient.com/XSExtensions"
	xmlns:soap="http://schemas.xmlsoap.org/wsdl/soap/" xmlns:soapenc="http://schemas.xmlsoap.org/soap/encoding/"
	xmlns:mime="http://schemas.xmlsoap.org/wsdl/mime/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	version="1.0">
	...
	<xsl:template name="schemaSet" match="//xs:schema">
		<SchemaSet schemaNamespace="{@targetNamespace}"
			qualifyAll="{@elementFormDefault = 'qualified'}">
			...
			<schemaImports>
				<xsl:for-each select=".//xs:import[parent::xs:schema][@namespace]">
					<schemaImport namespace="{@targetNamespace | @namespace}"
			...
			<schema>
				...
				<xsl:for-each
					select=".//xs:element[@name and xs:complexType]/xs:complexType">
				...
				<xsl:for-each
					select=".//xs:element[not(@name) and xs:complexType]/xs:complexType">
					<xsl:call-template name="makeDataSource">
						<xsl:with-param name="dsName"
							select="concat('autoDS',$startId + position())" />
					</xsl:call-template>
				</xsl:for-each>
	...
	<xsl:template name="webService" match="/wsdl:definitions">
		...
		<WebService dataURL="{.//wsdl:service//soap:address/@location}"
			name="{.//wsdl:service[//soap:address]/@name}" serviceNamespace="{@targetNamespace}">
			...
			<schemaImports>
				<xsl:for-each
					select="//xs:schema[@targetNamespace] |                                   //xs:import[parent::xs:schema][@namespace]">
					<schemaImport namespace="{@targetNamespace | @namespace}"
						location="{@schemaLocation}" />
				</xsl:for-each>
			</schemaImports>
						...
						<xsl:for-each select="./wsdl:operation">
							<operation name="{@name}" soapAction="{./soap:operation/@soapAction}">
								...
								<xsl:if test="wsdl:input/soap:body/@namespace">
                                  ...
                                  <xsl:value-of
										select="wsdl:input/soap:body/@namespace" />
                              </xsl:attribute>
								</xsl:if>
								...
								<xsl:if test="wsdl:output/soap:body/@namespace">
                                   ...
                                   <xsl:value-of
										select="wsdl:output/soap:body/@namespace" />
                               </xsl:attribute>
								</xsl:if>
                                ...
                                <xsl:value-of
									select="wsdl:input/soap:body/@use" />
                           </xsl:attribute>
								<xsl:attribute name="outputEncoding">
                               <xsl:value-of
									select="wsdl:output/soap:body/@use" />
                           </xsl:attribute>
                               ...
                               <xsl:value-of
									select="wsdl:input//soap:body/@parts" />
                           </xsl:attribute>
								<xsl:attribute name="outputParts">
                               <xsl:value-of
									select="wsdl:output//soap:body/@parts" />
                           </xsl:attribute>
					...
					<portType portTypeName="{@name}">
                       ...
                       <xsl:call-template name="getLocalName">
                          <xsl:with-param name="fullName"
									select="wsdl:input/@message" />
                       </xsl:call-template>
                    </xsl:attribute>
								<xsl:attribute name="outputMessage">
                       <xsl:call-template name="getLocalName">
                          <xsl:with-param name="fullName"
									select="wsdl:output/@message" />
                       </xsl:call-template>
                    </xsl:attribute>
							</operation>
						</xsl:for-each>
	...
	<xsl:template match="//xs:element[@name and @type and parent::xs:schema]">
		...
		<xsl:choose>
			...
			<xsl:when test="//xs:simpleType[@name = $typeName]">
				<SimpleType name="{@name}" xmlSource="XSElement"
			...
			<xsl:otherwise>
				<SimpleType name="{@name}" xmlSource="XSElement"
					inheritsFrom="{$typeName}" xsImportedType="true" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="makeDataSource">
		...
		<xsl:variable name="tagName">
			<xsl:choose>
				<xsl:when test="$xmlSource">
					<xsl:value-of select="$xmlSource" />
				</xsl:when>
				<xsl:when test="../wsdl:message">
					WSDLMessage
				</xsl:when>
		...
		<xsl:element name="{$tagName}">
			...
			<xsl:variable name="qualifyAll"
				select="ancestor::xs:schema/@elementFormDefault='qualified'" />


			<xsl:if
				test="$qualifyAll or                          $isGlobal or (../../xs:element and ../../../xs:schema)">
				<xsl:attribute name="mustQualify">true</xsl:attribute>
			</xsl:if>


			<xsl:variable name="thisDefinition" select="." />
			<xsl:variable name="immediateChildDefs"
				select="                 descendant::xs:element[ancestor::xs:complexType[1]=$thisDefinition]           " />
			<xsl:variable name="immediateChildAttributes"
				select="                 descendant::xs:attribute[ancestor::xs:complexType[1]=$thisDefinition]           " />
			...
			<xsl:if
				test="descendant::xs:any[ancestor::xs:complexType[1]=$thisDefinition]">
				<xsl:attribute name="xsdAnyElement">true</xsl:attribute>
			</xsl:if>


			<xsl:if
				test="xs:complexContent/xs:extension[@base!='anyType'] |                         xs:complexContent/xs:restriction[@base!='anyType']">
				<xsl:attribute name="inheritsFrom">
                    <xsl:call-template name="getLocalName">
                        <xsl:with-param name="fullName"
	...
	<xsl:template name="elementToField">
		...
		<xsl:param name="xmlRequired"
			select="                     (count(@minOccurs)=0 or @minOccurs &gt; 0) and not(../../xs:choice)" />
		...
		<xsl:variable name="useMaxOccurs"
			select="@maxOccurs &gt; 1 or @maxOccurs = 'unbounded'" />
		...
		<xsl:choose>
                   ...
                   <xsl:choose>
                      ...
                      <xsl:otherwise>
                         <xsl:call-template name="getAnonymousTypeId">
                            <xsl:with-param name="typeElement"
			...
			<xsl:when test="@type">
				...
				<xsl:variable name="complexTypeDef"
					select="//xs:complexType[@name = $typeName]" />
				...
				<xsl:if test="$complexTypeDef">
					<field name="{@name}" type="{$typeName}" xmlRequired="{$xmlRequired}">
						...
						<xsl:if
							test="                         contains($complexTypeDef/xs:complexContent/xs:restriction/@base,'Array')">
							<xsl:attribute name="multiple">true</xsl:attribute>
							<xsl:attribute name="type">
                          <xsl:call-template name="getISCType">
                             <xsl:with-param name="type"
								select="                                    substring-before(//@wsdl:arrayType,'[]')" />
                          </xsl:call-template>
                       </xsl:attribute>
						</xsl:if>
			...
			<xsl:when test="@ref or @element">
				...
				<xsl:variable name="refElement"
					select="//xs:element[@name = $ref and ../../xs:schema]" />
				...
				<xsl:choose>
					...
					<xsl:when test="$refElement/xs:complexType">
						<field name="{$refElement/@name}" type="{$refElement/@name}"
							xmlRequired="{$xmlRequired}" xsElementRef="true">
							<xsl:if test="$debug">
						...
						<xsl:if test="count(//xs:complexType[@name = $typeName]) = 0">
							<field xmlRequired="{$xmlRequired}" xsElementRef="true">
                                ...
                                <xsl:call-template
									name="getISCType">
                                    <xsl:with-param
									name="type" select="$refElement/@type" />
                                </xsl:call-template>
                            </xsl:attribute>
								...
								<xsl:if test="local-name()='part'">
                                    ...
                                    <xsl:value-of
										select="                                             $refElement/../../xs:schema/@targetNamespace" />
                                </xsl:attribute>
								</xsl:if>
								<xsl:if test="$partName">
									<xsl:attribute name="partName">
					...
					<xsl:when test="$refElement/xs:simpleType">
						...
						<xsl:for-each select="$refElement/xs:simpleType">
							...
							<field name="{$ref}" xmlRequired="{$xmlRequired}"
								xsElementRef="true">
								...
								<xsl:call-template name="getISCTypeAttribute">
									<xsl:with-param name="type"
										select="xs:restriction/@base | xs:list/@itemType" />
								</xsl:call-template>
					...
					<xsl:otherwise>
						...
						<field name="{$refLocal}" type="{$refLocal}" mustQualify="true"
							xmlRequired="{$xmlRequired}" xsElementRef="true">
			...
			<xsl:when test="xs:simpleType|xs:simpleContent">
				...
				<field name="{$fieldName}" xmlRequired="{$xmlRequired}">
					<xsl:variable name="type">
						<xsl:choose>
							<xsl:when
								test="                               not (xs:simpleType/xs:restriction/@base |                                    xs:simpleContent/xs:restriction/@base |                                    xs:simpleContent/xs:extension/@base)">
								string
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of
									select="xs:simpleType/xs:restriction/@base |                                                  xs:simpleContent/xs:restriction/@base |                                                  xs:simpleContent/xs:extension/@base" />
							</xsl:otherwise>
						</xsl:choose>
					</xsl:variable>
	...
	<xsl:template name="getISCType">
		...
		<xsl:choose>
			...
			<xsl:when test="$type='xs:NCName' or $type='xsd:NCName'">
				identifier
			</xsl:when>
	...
	<xsl:template name="simpleTypeContents">
		...
		<xsl:if
			test="xs:restriction/xs:minLength/@value &gt; 0 or                      xs:restriction/xs:length/@value &gt; 0">
		...
		<xsl:if
			test="count(xs:restriction/*) &gt; 0 and                       count(xs:restriction/*) !=                             count(xs:restriction/xs:attribute |                                   xs:restriction/xs:attributeGroup)">
			...
			<validators>
				...
				<xsl:variable name="rangeValidator">
					<xsl:choose>
						<xsl:when
							test="$localBase = 'decimal' or                                    $localBase = 'float' or                                   $localBase = 'double'">
							floatRange
						</xsl:when>
						<xsl:when
							test="$localBase = 'date' or                                    $localBase = 'datetime' or                                   $localBase = 'dateTime' or                                    $localBase = 'gYear' or                                    $localBase = 'gYearMonth'">
							dateRange
						</xsl:when>
						<xsl:otherwise>
							integerRange
				...
				<xsl:for-each select=".//*[not(local-name()=enumeration)]">
					<xsl:choose>
						...
						<xsl:when test="local-name()='fractionDigits'">
							<validator type="floatPrecision" precision="{@value}"
