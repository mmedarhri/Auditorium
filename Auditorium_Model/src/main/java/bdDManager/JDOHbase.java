...
public class JDOHbase
{
	...
	public Class getPersistable() {
		return persistable;
	}


	public void setPersistable(Class persistable) {
	...
	public void cleanOutPersonne()
	{
        ...
        {
            tx.begin();
            System.out.println("Supression de toutes les Personnes ");
            @SuppressWarnings("rawtypes")
			Extent prs=pm.getExtent(Personne.class,true);
            Extent dj=pm.getExtent(Demi_Journee.class,true);
           // Extent e1=pm.getExtent(Propriete.class,true);
            Query query=pm.newQuery(prs);
            Query query1=pm.newQuery(dj);
         
			Collection c=(Collection)query.execute();
            Collection c1=(Collection)query.execute();
      
            
			Iterator iter=c.iterator();
            while (iter.hasNext())
            {
             Personne pers = (Personne)iter.next();
             query.deletePersistentAll(pers);
            ...
            while (iter1.hasNext())
            {
             Demi_Journee demi_j = (Demi_Journee)iter1.next();
             query.deletePersistentAll(demi_j);
	...
	//cleanout Propriete table
	public void cleanOutPropriete()
	{
		pm = pmf.getPersistenceManager();
        tx = pm.currentTransaction();
        try
        {
            tx.begin();
            System.out.println("Supression de toutes les Proprietes ");
            @SuppressWarnings("rawtypes")
			Extent e_proprietes=pm.getExtent(Propriete.class,true);
            
            Query query_props=pm.newQuery(e_proprietes);
            //Query query = pm.newQuery("javax.jdo.query.JDOQL", "SELECT FROM datanucleusTutorial.core.User WHERE login == 'ff'");
          
            @SuppressWarnings("rawtypes")
			Collection c=(Collection)query_props.execute();
            //Collection c1=(Collection)query_filieres.execute();
           // System.out.println("collection is empty : "+c.isEmpty());
            @SuppressWarnings("rawtypes")
			Iterator iter=c.iterator();
            while (iter.hasNext())
            {
             Propriete prp = (Propriete)iter.next();
             query_props.deletePersistentAll(prp);               
            }
	...
	public void completeInfoUserByLogin(String login,String name,String prenom,String age,String adresse,String email)
	{
		pm = pmf.getPersistenceManager();
        tx = pm.currentTransaction();
        try
    ...
    public static void main(String args[]) throws FileNotFoundException, IOException
    {
	   ...
	   Personne p1 = new Personne(propriete);
	...
	/*		jdohb.persister(user1);
		...
		*/
		...
		Ressource rs1 = new Personne(candidat, aff);
		
		Audition audition = new Audition((Personne)rs1, dj);
